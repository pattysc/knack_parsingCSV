...
describe('dataHelpers Functions', function() {
  ...
  beforeEach(function() {
    ...
    helpers.findMultipleChoice(array)
    helpers.findDateAndText(array)
    columns = []
    headersArray = ["Sign Up", "ID", "Name"]
    helpers.makeColumns(headersArray, columns)
  })

  describe('makeColumns function', function(){

    it('should instantiate a Column object per header passed in', function() {
      expect(typeof(columns[0])).to.equal('object');
    })

    it('should a Column per header', function() {
      expect(headersArray.length).to.equal(columns.length);
    })

    it('should name each Column after a header', function() {
      expect(columns[0].name).to.equal('Sign Up');
      expect(columns[1].name).to.equal('ID');
      expect(columns[2].name).to.equal('Name');
    })
  }) //end of makeColumns
